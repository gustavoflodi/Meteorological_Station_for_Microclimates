...
## Roteiro para teste
Como mencionado anteriormente, não houveram muitas oportunidades para realizar os testes do projeto. Sendo assim, será descrito neste tópico um roteiro explicando os passos que foram planejados para fazer estes testes. O primeiro passo é compilar o código C que é responsável pela funcionalidade do sensor. Isso é feito com o comando **${CC} -Wall BMP280.c -o BMP280**, que produz um arquivo que pode ser executado direto na placa. Para conferir se o arquivo é compatível, usa se o comando **file BMP280**. Agora, é necessário transferir o arquivo para a placa com o comando **scp BMP280 root@"IP da placa":home/root**. Feito isso, podemos executar o arquivo pela linha de comando da máquina escrevendo **./BMP280**, que nos retornará os valores de temperatura e pressão medidos pelo sensor na tela do computador.

ALém disso, tem os testes da interface e do servidor. Primeiro é necessário verificar se tem Ptyhon instalado no computador. Caso não tenha, é necessário a senha do super usuário da máquina e instalar com o comando **python3 install pip PyQt5**, que pode variar de acordo com a versão do Python instalado. Em seguida, para rodar o servidor, basta mudar o IP dentro do código para ser o mesmo do computador e usar o comando **python3 Server_Python.py** . Para o arquivo em C, do cliente, é o mesmo processo de compilação do arquivo do sesnor BMP280. Vale lembrar que no código do cliente é necessário alterar o IP para ser da máquina também e configurar as portas de acrodo com as portas do arquivo PY. Por fim, para conectar ao servidor, é necessário clicar no botão indicado na interface gráfica antes de rodar o servidor.
